...
export default function TranscriberSettlementPanel({ refreshToken = 0, onPay, onChanged }: TranscriberSettlementPanelProps) {
  ...
  const handleResync = async () => {
    if (!window.confirm("완료 작업의 작업 시간과 정산 금액을 다시 계산하시겠습니까?")) return;
    setResyncing(true);
    setError(null);
    try {
      const result = await resyncSettlementSnapshots();
      await loadSnapshots();
      onChanged?.();
      window.alert(`정산을 다시 계산했습니다. (작업 시간 보정 ${result.duration_updated}건)`);
    } catch (err) {
      console.error(err);
      setError(err instanceof Error ? err.message : "정산 재계산에 실패했습니다.");
    } finally {
      setResyncing(false);
    }
  };
  ...
  return (
    <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex w-full max-w-[320px] flex-col gap-2 sm:flex-row sm:items-end">
          ...
          <button
            ...
            className="rounded-lg border border-cyan-500/30 bg-cyan-500/10 px-3 py-2 text-xs font-semibold text-cyan-300 disabled:cursor-not-allowed disabled:opacity-40"
          >
            {resyncing ? "재계산 중..." : "정산 재계산"}
          </button>
        </div>
      </div>
          ...
          <table className="w-full min-w-[1280px] border-collapse text-[13px]">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-950 text-left text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">
                ...
                <th className="px-3 py-2">분당단가</th>
              ...
              {visibleRows.map((row) => {
                ...
                return (
                  <tr key={row.transcriber_id} className="border-t border-slate-800 bg-slate-950/40 text-slate-300">
                    ...
                    <td className="px-3 py-2 text-slate-400">{formatCurrency(row.unit_price)}</td>
