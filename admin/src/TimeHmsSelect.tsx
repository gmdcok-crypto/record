...
const SELECT_CLASS =
  "rounded-lg border border-slate-700 bg-slate-950 px-2 py-2 text-sm text-slate-100 outline-none focus:border-cyan-500/50";
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <select value={value.hour} onChange={(event) => update({ hour: Number(event.target.value) })} className={SELECT_CLASS}>
        ...
        <select value={value.minute} onChange={(event) => update({ minute: Number(event.target.value) })} className={SELECT_CLASS}>
        ...
        <select value={value.second} onChange={(event) => update({ second: Number(event.target.value) })} className={SELECT_CLASS}>
