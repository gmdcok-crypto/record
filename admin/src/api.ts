...
export type ProxyUploadProject = {
  project_id: string;
  title: string;
  file_count?: number;
  completed_count?: number;
...
export async function resolveMemberForProxyUpload(params: {
  phone: string;
  name?: string;
  ensure?: boolean;
}): Promise<{ member: ProxyUploadMember; member_created: boolean }> {
  const res = await adminFetch(`${apiBase()}/api/admin/upload/resolve-member`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      phone: params.phone,
  ...
  if (!res.ok) throw await parseApiError(res, "회원 연결에 실패했습니다");
  return (await res.json()) as { member: ProxyUploadMember; member_created: boolean };
}

export async function fetchMemberProjectsForProxyUpload(memberId: number): Promise<ProxyUploadProject[]> {
  const res = await adminFetch(`${apiBase()}/api/admin/upload/members/${memberId}/projects`);
  if (!res.ok) throw await parseApiError(res, "프로젝트 목록을 불러올 수 없습니다");
  const data = (await res.json()) as { projects?: ProxyUploadProject[] };
  return data.projects ?? [];
}

export async function createMemberProjectForProxyUpload(
  memberId: number,
  title: string,
  memo?: string,
): Promise<ProxyUploadProject> {
  const res = await adminFetch(`${apiBase()}/api/admin/upload/members/${memberId}/projects`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ title, memo: memo ?? null }),
  });
  if (!res.ok) throw await parseApiError(res, "프로젝트 생성에 실패했습니다");
  const data = (await res.json()) as { project?: ProxyUploadProject };
  if (!data.project) throw new Error("프로젝트 생성에 실패했습니다");
  return data.project;
}

export async function uploadVoiceForMember(
  memberId: number,
  file: File,
  options?: {
    ...
    new Promise<ProxyUploadResult>((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          resolve(JSON.parse(xhr.responseText) as ProxyUploadResult);
          return;
        }
        try {
          const err = JSON.parse(xhr.responseText) as { detail?: unknown };
          reject(new Error(formatApiDetail(err.detail) || `업로드 실패 (${xhr.status})`));
        } catch {
  ...
  try {
    ...
    const presignRes = await fetch(`${base}/api/admin/upload/presign`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        member_id: memberId,
    ...
    if (!presignRes.ok || !presignData.upload_url || !presignData.job_id || !presignData.object_key) {
      throw new Error(formatApiDetail(presignData.detail) || "업로드 준비에 실패했습니다");
    }
    ...
    const completeRes = await fetch(`${base}/api/admin/upload/voice/complete`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        member_id: memberId,
    ...
    if (!completeRes.ok) {
      throw new Error(formatApiDetail(completeData.detail) || "업로드 완료 처리에 실패했습니다");
    }
    return completeData;
  } catch (directError) {
    console.warn("admin direct upload failed, falling back to backend upload", directError);
    try {
