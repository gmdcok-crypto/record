export type Hms = { hour: number; minute: number; second: number };

export const ZERO_HMS: Hms = { hour: 0, minute: 0, second: 0 };
...
export function hmsToMs({ hour, minute, second }: Hms): number {
  return (hour * 3600 + minute * 60 + second) * 1000;
}

export function clampHms(value: Hms, maxMs?: number | null): Hms {
...
export function sumSelectedSegmentDurationMs(
  segments: Array<{ selected: boolean; start_ms: number; end_ms: number }>,
): number {
  return segments
...
export const QUOTE_FILE_ACCEPT = "audio/*,video/mp4,video/webm,.wav,.mp3,.m4a,.flac,.ogg";
